...
namespace inviwo {
	...
	StreamlineIntegrator::StreamlineIntegrator()
		...
		, numberOfIntegration("numberOfIntegration", "Number Of Integration", 100, 0)
		, stepSize("stepSize", "Step Size", 0.1, 0, 10000)
		...
		if (propSeedMode.get() == 0) {
			...
			if (propMethod.get() == INTEGRATOR_EULER) {
				if (propStopingCondition.get() == INTEGRATOR_STOP_STEPS) {
					list = integrator.Euler_steps(vr, dims, pt, stepSize.get(), propIntegDirection.get(), propInDirectionField.get(), numberOfIntegration.get());
				}
				else if (propStopingCondition.get() == INTEGRATOR_STOP_ARC) {
					list = integrator.Euler_arc(vr, dims, pt, stepSize.get(), propIntegDirection.get(), propInDirectionField.get(), propArcLength.get());
				}
				else if (propStopingCondition.get() == INTEGRATOR_STOP_BOUNDARY) {
					list = integrator.Euler_boundary(vr, dims, pt, stepSize.get(), propIntegDirection.get(), propInDirectionField.get(), 0, 0, dims.x - 1, dims.y - 1);
				}if (propStopingCondition.get() == INTEGRATOR_STOP_SLOW) {
					list = integrator.Euler_slow(vr, dims, pt, stepSize.get(), propIntegDirection.get(), propInDirectionField.get(), propMinimalSpeed.get());
				}if (propStopingCondition.get() == INTEGRATOR_STOP_ZEROES) {
					list = integrator.Euler_zeroes(vr, dims, pt, stepSize.get(), propIntegDirection.get(), propInDirectionField.get());
				}
			}
			else if (propMethod.get() == INTEGRATOR_RK4) {
				if (propStopingCondition.get() == INTEGRATOR_STOP_STEPS) {
					list = integrator.RK4_steps(vr, dims, pt, stepSize.get(), propIntegDirection.get(), propInDirectionField.get(), numberOfIntegration.get());
				}
				else if (propStopingCondition.get() == INTEGRATOR_STOP_ARC) {
					list = integrator.RK4_arc(vr, dims, pt, stepSize.get(), propIntegDirection.get(), propInDirectionField.get(), propArcLength.get());
				}
				else if (propStopingCondition.get() == INTEGRATOR_STOP_BOUNDARY) {
					list = integrator.RK4_boundary(vr, dims, pt, stepSize.get(), propIntegDirection.get(), propInDirectionField.get(), 0, 0, dims.x - 1, dims.y - 1);
				}if (propStopingCondition.get() == INTEGRATOR_STOP_SLOW) {
					list = integrator.RK4_slow(vr, dims, pt, stepSize.get(), propIntegDirection.get(), propInDirectionField.get(), propMinimalSpeed.get());
				}if (propStopingCondition.get() == INTEGRATOR_STOP_ZEROES) {
					list = integrator.RK4_zeroes(vr, dims, pt, stepSize.get(), propIntegDirection.get(), propInDirectionField.get());
				}
